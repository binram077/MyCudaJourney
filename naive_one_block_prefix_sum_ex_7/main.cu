...
int main() {
    // Since we're assuming small array, set n to fit within a single block
    int n = 1024; // Maximum typical block size
    int blockSize = 1024; // Match block size to array size
    int numRuns = 100;
    ...
    int gridSize = 1; // Single block for naive implementation
    ...
    for (int i = 0; i < numRuns; i++) {
        ...
        prefixScan<<<gridSize, blockSize, 2 * n * sizeof(float)>>>(d_input, d_output, n);
    ...
    check_exclusive(h_input, h_output, n);
